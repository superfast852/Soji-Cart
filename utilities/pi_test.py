...
if __name__ == "__main__":
    print("__----- PI CART TESTING -----__\n")

    success = {"drive": 0, "arm": 0, "lidar": 0, "battery": 0, "ultrasonic": 0, "encoder": 0, "qmc5883l": 0}

    def test_class(test, params=""):
        checkmark = "\u2705"
        print("Test #1: Instantiation.")
        try:
            success[test] = eval((test.capitalize() if not test.startswith("qmc") else "QMC5883L") + "("+params+")")
        except Exception as e:
            success[test] = e
        print(f"{test.capitalize()}: {success[test] if isinstance(success[test], str) else checkmark}\n")
        print("Test #2. Testing Function.")
        if isinstance(success[test], str):
            print("Failed on Instantiation.")
        else:
            try:
                success[test] = esuccess[test].test
            except Exception as e:
                success[test] = e

            print(f"{test.capitalize()}: {checkmark if success[test] == 1 else success[test]}")


    test_class("drive", "11, 8, 7, 12")
    test_class("arm", "6")
    test_class("lidar", "'/dev/ttyUSB0'")
    test_class("battery")
    test_class("ultrasonic", "11, 8")
    test_class("encoder", "[[11, 8], [7, 12]]")
    test_class("magnetometer")
    test_class("qmc5883l")


    # TODO: Add more tests. Example: Usages, Error handling, Exiting.
